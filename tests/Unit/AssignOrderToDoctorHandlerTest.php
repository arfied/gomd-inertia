...
it('persists an OrderAssignedToDoctor event via the handler', function () {
    ...
    $handler = new AssignOrderToDoctorHandler($fakeStore, $fakeDispatcher);

    $command = new AssignOrderToDoctor(
        orderUuid: 'order-uuid-321',
        doctorId: 7,
        assignedByUserId: 99,
    ...
    expect($event)->toBeInstanceOf(OrderAssignedToDoctor::class)
        ->and($event->aggregateUuid)->toBe('order-uuid-321')
