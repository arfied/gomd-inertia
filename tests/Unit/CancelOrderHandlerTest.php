...
it('persists an OrderCancelled event via the handler', function () {
    ...
    $handler = new CancelOrderHandler($fakeStore, $fakeDispatcher);

    $command = new CancelOrder(
        orderUuid: 'order-uuid-789',
    ...
    expect($event)->toBeInstanceOf(OrderCancelled::class)
        ->and($event->aggregateUuid)->toBe('order-uuid-789')
